...
public class ShoppingCartGUI extends JFrame {
    ...
    public void addToCart(Product product) {
        ...
        if (cartItems.containsKey(product)) {
            ...
            price += (product.getProductPrice() * existingQuantity);
    ...
    private double calculateDiscount(double totalPrice) {
        // Apply a 20% discount if there are more than 3 items in any category
    ...
    private String getCategory(Product product) {
        if (product instanceof Electronics) {
            ...
            return "Other"; // Return a default category for unrecognized products
        }
    }
    ...
    private String getProductInfo(Product product) {
        if (product instanceof Electronics) {
            ...
            return "Size: " + clothing.getSize() + " inches, Color: " + clothing.getColor();
        } else {
            return "";
        }
    }
    ...
    private double calculateDiscount() {
        ...
        // Count the quantity of items in each category
        for (Product product : cartItems.keySet()) {
            String category = getCategory(product);
            categoryCount.put(category, categoryCount.getOrDefault(category, 0) + cartItems.get(product));
        }

        // Apply a 20% discount for each category with at least three items
        for (int count : categoryCount.values()) {
            if (count >= 3) {
                discount += calculateCategoryDiscount(count);
            }
        }
    ...
    // Method to calculate the discount for a specific category
    private double calculateCategoryDiscount(int quantity) {
        return quantity * 0.2 * getCategoryPrice(); // Assuming 20% discount per item in the category
    }

    // Method to get the price of a product in a category
    private double getCategoryPrice() {
        double totalPrice = 0;

        // Sum the prices of all items in the cart
        for (Product product : cartItems.keySet()) {
            totalPrice += product.getProductPrice() * cartItems.get(product);
        }

        // Calculate the average price
        double averagePrice = totalPrice / cartItems.size();

        // Alternatively, you could use a fixed price based on the category, depending on your requirements
        return averagePrice;
    }
